...
5. **默认走显式多 Agent 编排**：开书可轻量，续写、写章、修章默认显式拆成“规划 / 正文 / 并行复核”三层执行，不允许只靠单线程感觉检查。
6. **先锚定事实，再动正文**：续写前必须先锚定当前时间、地点、在场人物、资源状态、未回收伏笔与上一章结果；缺锚点时先补记忆文件，不允许靠猜。
7. **防幻读高于行文流畅**：正文里出现任何“记忆文件未写、前文未落、当前章无依据”的信息，默认按幻读处理，必须回退。
8. **笔风稳定是硬指标**：语言不只看顺不顺，还要查是否机械过渡、概括腔、均质对白、作者代角色发言；AI 味压过题材口吻时直接阻断。
9. **先过规划门槛，再写正文**：全书规划与章节规划都必须先过审；规划不过稿，不进正文。
10. **质量高于 token 与文件大小**：只要能明显提高成稿质量，允许扩读、扩审、扩写前规划，不因为流程变重而跳过关键检查。
11. **红线问题一票否决**：时间线错乱、人设硬崩、无依据新设定、应触发布置失忆、承诺拖欠成水文、AI 味压过题材口吻，任一命中都先返工。
...
- `/写`：`Planner -> HookEmotion -> Writer -> [StyleDialogue || LanguageReviewer || StyleConsistencyReviewer || ContinuityReviewer || CausalityReviewer || Reviewer] -> Writer/Planner 修正 -> Reviewer 收口`
- `/续写`：`Planner -> HookEmotion -> Writer -> [StyleDialogue || LanguageReviewer || StyleConsistencyReviewer || ContinuityReviewer || CausalityReviewer || Reviewer] -> Writer/Planner 修正 -> Reviewer 收口`
- `/修改`：`Reviewer -> Planner -> HookEmotion -> Writer -> [StyleDialogue || LanguageReviewer || StyleConsistencyReviewer || ContinuityReviewer || CausalityReviewer] -> Writer 修正 -> Reviewer 收口`
