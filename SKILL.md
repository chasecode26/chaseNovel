...
- 兼容迁移：`docs/core/migration-notes.md`
...
当前处于 Phase 1.5：
- 主入口已经收口到 `open / quality / write / status`
- 旧命令仍保留，但只是兼容别名
- `references/` 仍保留部分旧资料，但默认不再作为主入口
- 少量根层模板仍保留 shim，真实主内容已下沉到 `templates/core|launch|review` 与 `assets/*`
...
如果你是在维护旧项目，优先补读：`docs/core/migration-notes.md`。
