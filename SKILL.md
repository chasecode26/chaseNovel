...
10. 复核默认整章通读后再拆句，不做关键词检索式审稿。
11. 脚本、黑名单、闸门只算辅助证据；人眼读感不过，默认返工。
12. 起稿不准先写省力句、提炼句、抽象收口句，不准把“后面再修”当默认写法。
13. 默认先保质量，再保速度；不是先写省力版本再回头补质量。
...
- `Reviewer` 必须先整章读完，再按逻辑、严谨、重复、声口、作者发声、术语省略回头拆
- 命中硬伤后不能停在“指出问题”，必须交回给对应角色重写，再回到 `Reviewer` 复核
- 不能因为没命中几个黑名单词就判通过
...
- [rewrite-handoff.md](/D:/git/chaseNovel/templates/rewrite-handoff.md)
  `Reviewer` 判退后怎么把稿子交回去重写，避免只批不改
